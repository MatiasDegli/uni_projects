...
public class ArbolGen {
    ...
    */
    ...
    public Lista listaQueJustificaLaAltura(){
        ...
        alturaAux(raiz, camino, actual, alt, 0);

        return camino;
    }

    private void alturaAux(NodoGen recorre, Lista camino, Lista actual, int[] alturaMax, int alturaActual){
        
        if(recorre != null){  
            actual.insertar(alturaActual + 1, recorre.getElem());

            NodoGen hijo = recorre.getHijoIzquierdo();

            if(hijo == null && alturaActual > alturaMax[0]){

                Lista copia = actual.clone();                                
                camino.vaciar();
                
                for (int i = 1; i <= alturaActual; i++) {
                    camino.insertar(i, copia.recuperar(1));
                    copia.eliminar(1);    
                }
            ...
            while(hijo != null){
                alturaAux(hijo, camino, actual, alturaMax, alturaActual + 1);
                hijo = hijo.getHermanoDerecho(); 
            }

        }

        actual.eliminar(alturaActual + 1);
    }
